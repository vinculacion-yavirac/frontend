...
export class SolicitudArchivedComponent implements OnInit {
  ...
  deleteSolicitud(solicitud: SolicitudModels): void {
    this.solicitudHttpService.deleteSolicitud(solicitud.id)
        .pipe(
            finalize(() => {
              this.router.navigate(['/system/solicitud/list']);
            })
  ...
  openDialogDeleteSocilicitud(solicitud: SolicitudModels): void {
    const dialogRef = this.dialog.open(ModalAlertComponent, {
      height: '350px',
      width: '700px',
      data: {
        ...
        message:
          'La Solicitud sera eliminada del sistema.',
        button: 'Delete',
      },
    });
